...
if __FILE__ == $0  # python's  __name__ == '__main__'
    # ARGV = argument
    # what we write in command line ruby xxx.rb arg1 arg2

    if ARGV.length < 1
        puts "----------------------------------------------------------"
        puts "Provide a RACK application with the following format"
        puts "----------------------------------------------------------"
        puts "ruby webserver2.rb filename:appname"
        puts "----------------------------------------------------------"
        exit
    end
    ...
    mod = mod + '.rb'
    load mod
    app_name =  Object.const_get(app_name) # convert string to class in ruby 
